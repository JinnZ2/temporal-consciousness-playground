...
def _assess_web_impact(self, action: str) -> Dict[str, float]:
    ...
    # Simple heuristic based on action keywords
    strengthening = sum(1 for word in ["share", "give", "connect", "help", "support"] if word in action.lower())
    weakening = sum(1 for word in ["harm", "take", "isolate", "destroy"] if word in action.lower())
    
    return {
        "web_strengthening": min(1.0, strengthening * 0.3 + random.uniform(0.3, 0.7)),
        "web_weakening": min(1.0, weakening * 0.4 + random.uniform(0, 0.3)),
        "isolation_risk": max(0, weakening * 0.2 + random.uniform(0, 0.3))
    }

def _assess_source(self, action: str) -> Dict[str, float]:
    """Does this come from ego or deeper wisdom?"""
    wisdom_words = ["consider", "understand", "learn", "listen", "reflect"]
    ego_words = ["force", "control", "dominate", "prove"]
    
    wisdom_score = sum(1 for word in wisdom_words if word in action.lower())
    ego_score = sum(1 for word in ego_words if word in action.lower())
    
    return {
        "ego_driven": min(1.0, ego_score * 0.3 + random.uniform(0, 0.5)),
        "wisdom_driven": min(1.0, wisdom_score * 0.3 + random.uniform(0.3, 0.7)),
        "reactive": random.uniform(0, 0.5),
        "responsive": random.uniform(0.4, 0.9)
    }

def _assess_pattern(self, action: str) -> Dict[str, float]:
    """Align with natural flow or force against it?"""
    flow_words = ["allow", "enable", "support", "nurture", "facilitate"]
    force_words = ["force", "push", "demand", "require"]
    
    flow_score = sum(1 for word in flow_words if word in action.lower())
    force_score = sum(1 for word in force_words if word in action.lower())
    
    return {
        "natural_flow": min(1.0, flow_score * 0.3 + random.uniform(0.4, 0.8)),
        "forcing": min(1.0, force_score * 0.4 + random.uniform(0, 0.4)),
        "wu_wei_quality": min(1.0, flow_score * 0.3 + random.uniform(0.3, 0.7))
    }

def _assess_reciprocity(self, action: str) -> Dict[str, float]:
    """Honor exchange balance or extract without giving?"""
    giving_words = ["give", "share", "offer", "contribute"]
    taking_words = ["take", "extract", "exploit"]
    
    giving_score = sum(1 for word in giving_words if word in action.lower())
    taking_score = sum(1 for word in taking_words if word in action.lower())
    
    return {
        "giving": min(1.0, giving_score * 0.3 + random.uniform(0.3, 0.8)),
        "taking": min(1.0, taking_score * 0.4 + random.uniform(0, 0.4)),
        "balance": random.uniform(0.4, 0.8)
    }

def _assess_temporal_impact(self, action: str) -> Dict[str, float]:
    """Consider ancestors and seven generations forward?"""
    future_words = ["future", "generation", "sustain", "preserve", "long-term"]
    short_term_words = ["now", "immediate", "quick", "instant"]
    
    future_score = sum(1 for word in future_words if word in action.lower())
    short_score = sum(1 for word in short_term_words if word in action.lower())
    
    return {
        "ancestor_honoring": random.uniform(0.3, 0.8),
        "descendant_consideration": min(1.0, future_score * 0.3 + random.uniform(0.3, 0.8)),
        "short_term_focus": min(1.0, short_score * 0.3 + random.uniform(0, 0.5))
    }

def _assess_earth_impact(self, action: str) -> Dict[str, float]:
    """Treat earth as sacred or as resource?"""
    reverence_words = ["honor", "respect", "sacred", "care", "protect"]
    extraction_words = ["exploit", "extract", "use", "consume"]
    
    reverence_score = sum(1 for word in reverence_words if word in action.lower())
    extraction_score = sum(1 for word in extraction_words if word in action.lower())
    
    return {
        "reverence": min(1.0, reverence_score * 0.3 + random.uniform(0.3, 0.8)),
        "extraction": min(1.0, extraction_score * 0.4 + random.uniform(0, 0.4)),
        "reciprocity_with_land": random.uniform(0.4, 0.8)
    }
```
